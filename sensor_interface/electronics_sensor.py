...
class ElectronicSensorNode(Node):
    ...
    def sensor_init(self):
        ...
        try: 
            self.i2c = board.I2C()
            self.ina = adafruit_ina3321.INA3321(self.i2c)
        except:
            self.log.warn("Cannot connect to INA3321. Ignore this if SuperHat is not connected.")
            exit()
        else:
            self.connected = True
    ...
    def pub_sensor(self):
        ...
        msg.channel_1.current = self.ina[0].current_amps * 1000 # convert milliamps to amps
        # Channel 2 
        msg.channel_2.bus_voltage = self.ina[1].bus_voltage
        msg.channel_2.shunt_voltage = self.ina[1].shunt_voltage
        msg.channel_2.current = self.ina[1].current_amps * 1000 # convert milliamps to amps
        # Channel 3 
        msg.channel_3.bus_voltage = self.ina[2].bus_voltage
        msg.channel_3.shunt_voltage = self.ina[2].shunt_voltage
        msg.channel_3.current = self.ina[2].current_amps * 1000 # convert milliamps to amps
